...
func (s *Storage) GetMessagesByChatID(chatID int) ([]domain.Message, error) {
	messageRows, err := s.db.Query(
		"SELECT m.id, m.user_id, m.content, m.created_at, u.username FROM messages m JOIN users u ON m.user_id = u.id WHERE chat_id = $1 ORDER BY m.created_at",
		chatID,
	)
	if err != nil {
		return nil, err
...
func (s *Storage) GetChatsByUserID(userID int) ([]domain.Chat, error) {
	rows, err := s.db.Query(`
	SELECT c.id, 
	       CASE 
